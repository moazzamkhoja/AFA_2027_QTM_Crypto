...
\begin{table}[ht]
\centering
\caption{Spanning Tests: Token Conviction Quintile vs.\ the Completed Battery}
\label{tab:spanning}
\small
\begin{tabular}{lccc}
\toprule
...
MA-distance long-short & 0.0183** & (2.46) & 50 \\
Volatility long-short & 0.0169** & (2.10) & 50 \\
Idiosyncratic-volatility long-short & 0.0169** & (2.15) & 50 \\
Amihud illiquidity long-short & 0.0174** & (2.14) & 50 \\
Skewness long-short & 0.0166** & (2.11) & 49 \\
P/F long-short & 0.0122 & (1.67) & 41 \\
Revenue-DCF long-short & 0.0154* & (1.84) & 38 \\
\midrule
Momentum family + size (four) & 0.0234** & (2.60) & 50 \\
Completed battery (twelve long-shorts) & 0.0174** & (2.45) & 43 \\
Completed battery + fee comparators (fourteen) & 0.0217*** & (2.82) & 35 \\
\bottomrule
\end{tabular}
\smallskip
...
token class-month) on each signal. The continuous MA-distance signal supersedes a
binary MA-crossover specification from an earlier draft of the battery, whose long-short
legs were populated in only 28--31 months and rendered two cells underpowered; with
full month overlap restored, those cells are significant. The P/F and revenue-DCF rows
run on the fee-covered subsamples (41 and 38 months); their lower alphas reflect
sample composition, with small and insignificant competitor loadings, rather than
spanning. In the reverse direction, no competitor long-short earns positive alpha on
the LTW factors plus the conviction quintile.}
\end{table}
...
Table~\ref{tab:horserace} reports the signal-by-signal race;
Table~\ref{tab:altport} reports each alternative signal as a long-short portfolio.
Four results matter. First, the coin interaction survives the entire completed
battery: with all twelve comparators in the regression, conviction $\times$ valuation
remains $-2.35\%$ ($t=-2.68$) over the full sample and $-1.88\%$ ($t=-3.17$)
post-2024.

\begin{table}[ht]
\centering
\caption{Horse Race: Alternative Signals and One-Month-Ahead Returns}
\label{tab:horserace}
\footnotesize
...
\begin{table}[ht]
\centering
\caption{Alternative Signals as Long-Short Portfolios (Tokens)}
\label{tab:altport}
\small
\begin{tabular}{lccccc}
\toprule
Quintile long-short & Mean/mo & $\alpha$/mo & $t$ & Sharpe & Months \\
\midrule
P/F, cheap $-$ expensive & 0.0079 & $-$0.0018 & ($-$0.10) & 0.15 & 41 \\
Revenue DCF, cheap $-$ expensive & $-$0.0060 & 0.0098 & (0.55) & $-$0.23 & 38 \\
MA-distance, high $-$ low & $-$0.0232 & $-$0.0217 & ($-$1.57) & $-$0.86 & 50 \\
Volatility, high $-$ low & $-$0.0125 & $-$0.0106 & ($-$0.73) & $-$0.48 & 50 \\
Idiosyncratic vol, high $-$ low & $-$0.0124 & $-$0.0056 & ($-$0.43) & $-$0.52 & 50 \\
Amihud, illiquid $-$ liquid & $-$0.0065 & $-$0.0147 & ($-$0.82) & $-$0.25 & 50 \\
Skewness, high $-$ low & $-$0.0054 & 0.0031 & (0.16) & $-$0.22 & 49 \\
\midrule
Conviction quintile (reference) & 0.0228 & 0.0171** & (2.18) & 0.86 & 50 \\
\bottomrule
\end{tabular}
\smallskip

\parbox{0.9\linewidth}{\footnotesize
\textit{Notes:} Construction identical to the conviction quintile
(Table~\ref{tab:quintile}): EW, minimum 3 names per leg, monthly rebalancing, NW-3
alphas vs.\ the monthly LTW factors. No alternative signal --- including the two
fee-based valuation ratios --- earns a significant long-short alpha in either
direction; P/F's panel-level significance does not translate into a portfolio at
$\sim$5--6 names per leg on the half-covered sample. No cell that is null in the full
sample revives post-2023.}
\end{table} Amihud illiquidity is the strongest new coin-level single (illiquid coins
underperform, $-2.23\%$ per SD, $t=-2.65$), but it does not dent the interaction.
Second, the token conviction \textit{slope} does not survive the joint race: it
attenuates to $+0.3\%$ ($t \approx 1.1$), while short-horizon reversal remains the
dominant token signal ($-1.66\%$, $t=-2.46$). Combined with
...
cross-sectional slope. Third --- a result of independent interest --- the practitioner
fee multiple is the one valuation comparator that works in the token panel: ln P/F
predicts returns at $-1.03\%$ per SD ($t=-2.50$; $-1.48\%$, $t=-2.70$ post-2024) and
survives the completed joint battery ($-0.90\%$, $t=-2.32$), where raw NV/TVL never
did. Yet its own DCF refinement destroys it (the growth-levelized revenue and fee
versions are dead, $t=-0.42$ and $-0.68$), and it earns nothing as a quintile
portfolio ($t=-0.10$ at five to six names per leg): a panel-slope phenomenon without
portfolio translation --- the mirror image of conviction. Growth-levelization, in
other words, adds value where the fundamental is a throughput flow (coins) and
subtracts it where the fundamental is already a cash flow (fees). The fee multiple
neither spans nor revives conviction: the conviction slope on the fee-covered
subsample is unchanged by the P/F column ($+0.24 \to +0.19$). Fourth, the scarcity
narratives popular in practitioner work find little support: stock-to-flow is
insignificant everywhere, and supply growth matters only for coins post-2024
($-1.43\%$ per SD, $t=-2.79$). A descriptive Metcalfe panel (available for seven
majors only) confirms the well-known Bitcoin price-to-Metcalfe mean reversion
($t=-3.69$ in-sample) but cannot enter the cross-sectional race for coverage reasons.
...
H1a: coin conviction level & Rejected at $t{+}1$ & $t=0.69$ alone; marginally positive at $t{+}6$ ($t=1.90$) \\
H1b: token conviction level & Qualified support & $+0.6$--$0.8\%$/mo per SD, $t=1.5$--$2.3$; attenuates under sector FE and in the joint race \\
H1b refinement: voting-weighted $\lambda$ & Not supported & $t=-0.48$ on a low-power subsample \\
...
Structure of the premium & Between sectors, in the extremes & sector-neutral and decile sorts die; per-sector sorts null; no absorption proxy conditions it \\
\midrule
