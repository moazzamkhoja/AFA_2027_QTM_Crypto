...
without coarse protocol-sector fixed effects, the panel slope sits at $t \approx 1.5$.
...
within the token universe requires care. The sector-neutralized quintile dies
($t=0.39$), coarse-sector fixed effects attenuate the regression slope
(Table~\ref{tab:token_ladder}), and per-sector \textit{tercile} long-shorts are
uniformly insignificant (DEX $t=0.89$; Lending $t=-0.16$; Other $t=-0.77$) ---
evidence that at first suggested a purely between-category effect. But the tercile is
the wrong instrument: it dilutes a signal that lives in the tails, exactly as it does
in the full cross-section. In the one category deep enough to support a within-sector
\textit{quintile} --- DEX, with 39 tokens and a median of 25 per month --- the
conviction long-short earns $+3.4\%$ per month ($t=2.04$) on legs of fewer than five
names. The correct statement is therefore that the premium is present within the
deepest category at fine granularity; the remaining categories are too thin to test at
that granularity, and the pooled sector-neutralized construction fails because
demeaning within thin categories injects noise. The value-weighted variant adds a
